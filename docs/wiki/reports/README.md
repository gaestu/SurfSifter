...
- Tag Summary XLSX export (📝 button in the Reports tab toolbar) produces a printable working document grouped by tag and artifact group with a ☐ checkbox per group; saved by default under `<case>/reports/tag_summary_<evidence>.xlsx`, restricted to the case workspace, and recorded in `process_log` with the completion timestamp/provenance.
