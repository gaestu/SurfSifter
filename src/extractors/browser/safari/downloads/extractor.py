"""
Safari Downloads Extractor.

Extracts download history from Safari on macOS.
Uses plist format (Downloads.plist).
...
from .._parsers import parse_downloads, get_download_stats
...
LOGGER = get_logger("extractors.browser.safari.downloads")


class SafariDownloadsExtractor(BaseExtractor):
    """
    Extract Safari downloads from macOS evidence.

    Parses Downloads.plist file.

    Note: Safari only stores completed downloads in Downloads.plist.
    Unlike Chromium, there's no tracking of in-progress or failed downloads.
    """
    ...
    def metadata(self) -> ExtractorMetadata:
        ...
        return ExtractorMetadata(
            ...
            display_name="Safari Downloads (macOS)",
            description="Extract Safari download history from macOS - EXPERIMENTAL",
            category="browser",
            requires_tools=[],
            can_extract=True,
            can_ingest=True,
    ...
    def get_status_widget(
        ...
        if manifest_path.exists():
            try:
                ...
                downloads = parsed.get("downloads", 0)
                ...
                status_text = (
                    f"Safari Downloads (EXPERIMENTAL)\n"
                    f"Files: {file_count}\n"
                ...
                status_text = "Safari Downloads - Error reading manifest"
        else:
            status_text = "Safari Downloads (EXPERIMENTAL)\nNo extraction run yet"
    ...
    def get_output_dir(
        ...
        """Return output directory for Safari downloads extraction."""
        return case_root / "evidences" / evidence_label / "safari_downloads"
    ...
    def run_extraction(
        ...
        """Extract Safari Downloads.plist files from evidence."""
        ...
        callbacks.on_step("Initializing Safari downloads extraction")
        LOGGER.info("Starting Safari downloads extraction (run_id=%s)", run_id)
        ...
        if not files_by_partition:
            LOGGER.info("No Safari downloads files found")
            manifest_data["notes"].append("No Safari downloads files found")
        else:
            callbacks.on_step(f"Extracting {total_discovered} Safari downloads files")
        ...
        LOGGER.info("Safari downloads extraction complete: %d files", len(manifest_data["files"]))
    ...
    def run_ingestion(
        ...
        """Parse extracted Safari downloads and ingest into database."""
        from core.statistics_collector import StatisticsCollector
        from core.database import insert_browser_downloads
        ...
        if not manifest_path.exists():
            ...
            return {"downloads": 0, "total_bytes": 0}
        ...
        if not files:
            callbacks.on_log("No Safari downloads files to ingest", "warning")
            if collector:
        ...
        for file_info in files:
            ...
            try:
                ...
                downloads = parse_downloads(local_path)
                ...
                for dl in downloads:
                    ...
                    if dl.url:
                        ...
                        url_records.append({
                            ...
                            "run_id": run_id,
                            "source_path": source_path,
                            "context": f"download:safari:{profile}",
                            "first_seen_utc": None,  # Safari doesn't store download timestamps
                        })
                ...
                callbacks.on_log(f"Parsed {len(downloads)} downloads from {profile}", "info")

                update_inventory_ingestion_status(
                    evidence_conn,
                    inventory_id=inventory_id,
                    status="ok",
                    records_parsed=len(downloads),
                )
        ...
        inserted = 0
        if all_records:
            callbacks.on_step("Inserting download records")
            inserted = insert_browser_downloads(evidence_conn, evidence_id, all_records)
            evidence_conn.commit()
            callbacks.on_log(f"Ingested {inserted} downloads", "info")
        ...
        return {"downloads": total_downloads, "total_bytes": total_bytes}
