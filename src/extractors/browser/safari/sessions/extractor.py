...
class SafariSessionsExtractor(BaseExtractor):
    ...
    def run_extraction(
        ...
        for artifact_name in ("sessions", "recently_closed_tabs"):
            for pattern in get_patterns(artifact_name):
                try:
        ...
        if not discovered_files:
            ...
            callbacks.on_step(f"Extracting {len(discovered_files)} Safari session files")
            ...
            for path_str in discovered_files:
                ...
                try:
                    file_info = self._extract_file(evidence_fs, path_str, files_dir)
                    if file_info:
