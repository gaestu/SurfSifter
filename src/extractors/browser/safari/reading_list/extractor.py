"""Safari Reading List extractor implementation."""
...
from .._parsers import SafariReadingListItem, get_reading_list_stats, parse_reading_list
from .._patterns import extract_user_from_path

LOGGER = get_logger("extractors.browser.safari.reading_list")


class SafariReadingListExtractor(BaseExtractor):
    """Extract Safari Reading List entries from macOS evidence."""

    @property
    def metadata(self) -> ExtractorMetadata:
        return ExtractorMetadata(
            name="safari_reading_list",
            display_name="Safari Reading List",
            description="Extract Safari Reading List entries from Bookmarks.plist",
            category="browser",
            requires_tools=[],
            can_extract=True,
            can_ingest=True,
    ...
    def get_status_widget(
        ...
        manifest_path = self._find_latest_manifest(output_dir)
        if not manifest_path:
            return QLabel("Safari Reading List\nNo extraction run yet", parent)
        try:
            manifest = json.loads(manifest_path.read_text())
            ...
            text = (
                ...
                f"Bookmarks.plist Files: {file_count}\n"
                f"Reading List Items: {parsed.get('total_items', 0)}\n"
                f"Unique URLs: {parsed.get('unique_urls', 0)}"
            )
        except Exception:
            text = "Safari Reading List\nFailed to read manifest"
    ...
    def get_output_dir(
        ...
        return case_root / "evidences" / evidence_label / "safari_reading_list"
    ...
    def run_extraction(
        ...
        callbacks.on_step("Initializing Safari reading list extraction")
        LOGGER.info("Starting Safari reading list extraction (run_id=%s)", run_id)
        ...
        if not files_by_partition:
            ...
            manifest_data["notes"].append("No Safari Bookmarks.plist files found")
        else:
        ...
        callbacks.on_step("Safari reading list extraction complete")
    ...
    def run_ingestion(
        ...
        if not files:
            if collector:
                collector.report_ingested(evidence_id, self.metadata.name, records=0, reading_list=0, urls=0)
                collector.finish_run(evidence_id, self.metadata.name, status="success")
            return counts

        callbacks.on_step("Initializing Safari reading list ingestion")
        delete_bookmarks_by_run(evidence_conn, evidence_id, run_id)
        delete_urls_by_run(evidence_conn, evidence_id, run_id)

        bookmark_records: List[Dict[str, Any]] = []
        url_records: List[Dict[str, Any]] = []
        all_items: List[SafariReadingListItem] = []

        for file_info in files:
            if file_info.get("artifact_type") != "reading_list_plist":
                continue
            ...
            try:
                ...
                items = parse_reading_list(local_path_obj)
                all_items.extend(items)

                urls_parsed = 0
                for item in items:
                    # Build notes JSON with Reading List metadata
                    notes_data: Dict[str, Any] = {}
                    if item.preview_text:
                        notes_data["preview_text"] = item.preview_text
                    if item.fetch_result is not None:
                        notes_data["fetch_result"] = item.fetch_result
                    if item.date_last_fetched_utc:
                        notes_data["date_last_fetched"] = item.date_last_fetched_utc
                    if item.date_last_viewed_utc:
                        notes_data["date_last_viewed"] = item.date_last_viewed_utc
                    notes_str = json.dumps(notes_data) if notes_data else None
                    ...
                    bookmark_records.append({
                        ...
                        "url": item.url,
                        "title": item.title,
                        "folder_path": "com.apple.ReadingList",
                        "date_added_utc": item.date_added_utc,
                        "bookmark_type": "reading_list",
                        "run_id": run_id,
                        ...
                        "logical_path": source_path,
                        "forensic_path": source_path,
                        "discovered_by": "safari_reading_list",
                        "notes": notes_str,
                    })
                    ...
                    url_record = self._make_url_record(
                        ...
                        source_path=source_path,
                        profile=profile,
                        first_seen_utc=item.date_added_utc,
                    )
                    if url_record:
        ...
        if bookmark_records:
            counts["reading_list"] = insert_bookmarks(evidence_conn, evidence_id, bookmark_records)
        if url_records:
            counts["urls"] = insert_urls(evidence_conn, evidence_id, url_records)

        item_stats = get_reading_list_stats(all_items)
        ...
        callbacks.on_step("Safari reading list ingestion complete")

        if collector:
            collector.report_ingested(
                evidence_id,
                self.metadata.name,
                records=counts["reading_list"],
                reading_list=counts["reading_list"],
                urls=counts["urls"],
            )
            status = "success" if counts["reading_list"] > 0 else "partial"
            collector.finish_run(evidence_id, self.metadata.name, status=status)
    ...
    def _extract_file(
        ...
        if partition_index is not None:
            base_name = f"safari_{_safe_slug(profile)}_p{partition_index}_Bookmarks.plist"
        else:
            base_name = f"safari_{_safe_slug(profile)}_Bookmarks.plist"
        dest_path = run_dir / base_name
        if dest_path.exists():
            suffix = hashlib.sha1(source_path.encode("utf-8", errors="ignore")).hexdigest()[:8]
            dest_path = run_dir / f"safari_{_safe_slug(profile)}_{suffix}_Bookmarks.plist"
        ...
        return {
            ...
            "source_path": source_path,
            "artifact_type": "reading_list_plist",
            "browser": "safari",
            "user": user,
            "profile": profile,
    ...
    def _make_url_record(
        *,
        ...
        if lowered.startswith(("about:", "safari-", "javascript:", "data:")):
            return None
        ...
        return {
            ...
            "run_id": run_id,
            "source_path": source_path,
            "context": f"reading_list:safari:{profile}",
