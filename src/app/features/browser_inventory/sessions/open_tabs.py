"""Open Tabs subtab widget for session restore data."""
...
from .dialog import SessionDetailsDialog
from .model import SessionTabsTableModel

logger = logging.getLogger(__name__)


class OpenTabsSubtab(BaseArtifactSubtab):
    """Browser session tabs with browser filtering."""

    def _default_status_text(self):
        return "0 session tabs"

    def _setup_filters(self, fl: QHBoxLayout):
        self.browser_filter = self._add_browser_filter(fl)

    def _create_model(self):
        return SessionTabsTableModel(
            self.ctx.db_manager,
    ...
    def _configure_table(self):
        ...
        t.setColumnWidth(4, 80)    # Window
        t.setColumnWidth(5, 60)    # Tab
        t.setColumnWidth(6, 50)    # Pinned
        t.setColumnWidth(7, 120)   # Last Accessed
    ...
    def _update_status(self):
        ...
        windows = self._model.get_window_count()
        pinned = self._model.get_pinned_count()
        self.status_label.setText(
            f"{count} tabs in {windows} windows ({pinned} pinned)"
        )
