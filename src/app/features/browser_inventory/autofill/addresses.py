...
from .profile_tokens_model import AutofillProfileTokensTableModel
from .dialog import AutofillDetailsDialog

logger = logging.getLogger(__name__)


class AddressesSubtab(BaseArtifactSubtab):
    """Chromium autofill profile address tokens with browser/field filtering."""

    def _default_status_text(self):
        return "0 address tokens"

    def _setup_filters(self, fl: QHBoxLayout):
        self.browser_filter = self._add_browser_filter(fl)

        fl.addWidget(QLabel("Field Type:"))
        self.field_filter = QLineEdit()
        self.field_filter.setPlaceholderText("Filter by field type...")
        self.field_filter.setMaximumWidth(200)
        fl.addWidget(self.field_filter)

    def _create_model(self):
        return AutofillProfileTokensTableModel(
            self.ctx.db_manager,
    ...
    def _configure_table(self):
        t = self.table
        t.setColumnWidth(0, 100)   # Profile GUID
        t.setColumnWidth(1, 70)    # Token Type
        t.setColumnWidth(2, 140)   # Field Type
        t.setColumnWidth(3, 200)   # Value
        t.setColumnWidth(4, 80)    # Browser
        t.setColumnWidth(5, 80)    # Profile
        t.setColumnWidth(6, 100)   # Modified
    ...
    def _apply_filters(self):
        ...
        field_type = self.field_filter.text().strip()
        self._model.load(browser_filter=browser, field_type_filter=field_type)
        self._update_status()
    ...
    def _update_status(self):
        ...
        self.status_label.setText(f"{count} address tokens")

    def _artifact_type_for_tagging(self):
        return "autofill_profile_token"
