...
from .dialog import MediaDetailsDialog
from .model import MediaHistoryTableModel

logger = logging.getLogger(__name__)


class MediaSubtab(BaseArtifactSubtab):
    """Media playback history with browser filtering."""

    def _default_status_text(self):
        return "0 media records"

    def _setup_filters(self, fl: QHBoxLayout):
        self.browser_filter = self._add_browser_filter(fl)

    def _create_model(self):
        return MediaHistoryTableModel(
            self.ctx.db_manager,
    ...
    def _configure_table(self):
        ...
        t.setColumnWidth(1, 80)    # Browser
        t.setColumnWidth(2, 80)    # Profile
        t.setColumnWidth(3, 100)   # Watch Time
        t.setColumnWidth(4, 50)    # Video
        t.setColumnWidth(5, 50)    # Audio
        t.setColumnWidth(6, 120)   # Last Updated
    ...
    def _update_status(self):
        ...
        total_time = self._model.get_total_watch_time()
        video_count = self._model.get_video_count()
        time_str = MediaHistoryTableModel.format_watch_time(total_time)
        self.status_label.setText(
            f"{count} records ({time_str} total, {video_count} with video)"
        )
    ...
    def _view_details(self, index):
        ...
        if not row_data:
            return
        MediaDetailsDialog(row_data, parent=self).exec()
